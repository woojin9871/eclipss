...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	// 데이터 소스 객체
	// app.property에 입력한 datasource 정보
	@Autowired
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests() // 인가 설정
			...
			.antMatchers("/").permitAll() // "/" 경로는 모든 사용자에 대해 허용
...
//		http.csrf().disable();
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		// JDBC 인증 세팅 정보
		// - 데이터 소스
		// - 아이디/비밀번호/활성여부를 조회하는 쿼리 (인증 처리)
		// - 아이디/권한을 조회하는 쿼리 (인가 처리)
		// - 비밀번호 암호화 방식
		...
		auth.jdbcAuthentication() // 인증방식 : JDBC 방식으로 지정(데이터베이스에 등록된 사용자 정보로 인증)
